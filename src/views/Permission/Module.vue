<template>
  <section>
    <!--工具条-->
    <el-col :span="24" class="toolbar" style="padding-bottom: 0px;margin-left: 20px;">
    ...
    >
      <el-table-column
        ...
        label="父级接口名称"
      ></el-table-column>
      <el-table-column prop="children" label="子级接口数据">
    ...
    >
      ...
      >
        ...
        <el-form-item label="父级名称" prop="ParentId">
          <el-select v-model="editForm.ParentId" placeholder="请选择父级">
            <el-option
              v-for="item in mparentData"
              :key="item.Id"
              :label="item.IName"
              :value="item.Id"
            >
    ...
    >
      ...
      >
        ...
        <el-form-item label="父级名称" prop="ParentId">
          <el-select v-model="addForm.ParentId" placeholder="请选择父级">
            <el-option
              v-for="item in mparentData"
              :key="item.Id"
              :label="item.IName"
  ...
  methods: {
    ...
    getRoles() {
      ...
      getModuleListPage(para).then((res) => {
        ...
        // this.interfaces = res.data.response.data;
        this.listLoading = false;
        //NProgress.done();
      });
      getTableTreeModuleListPage().then((res) => {
        this.interfaces = res.data.response;
      });
    },
    //删除
    ...
    handleEdit: function (index, row) {
      ...
      getMParent().then((res) => {
        this.mparentData = res.data.response.Data;
      });
    },
    //显示新增界面
    async handleAdd() {
